...
class RDojo_UI:
    ...
    def ui(self, *args):
        ...
        self.UIElements['mainColLayout'] = cmds.columnLayout( adjustableColumn=True, bgc=[0.0, 1.0, 0.0] )

        self.UIElements['guiFrameLayout1'] = cmds.frameLayout( label='Layout', borderStyle='out', bgc=[1.0, 0.0, 0.0],
            p=self.UIElements['mainColLayout'] )

        self.UIElements['guiFlowLayout1'] = cmds.flowLayout( v=False, width=window_width, height=window_height/2, wr=True, bgc=[0.0, 0.0, 1.0],
            p=self.UIElements['guiFrameLayout1'])

        self.UIElements['rowColumnLayout1'] = cmds.rowColumnLayout( numberOfColumns=3, columnAlign=[ (1, 'left'), (2, 'right'), (3, 'right')], 
            columnWidth=[(1, 120), (2, 120), (3, 120)], bgc=[1.0, 1.0, 0.0], p=self.UIElements['mainColLayout'] )
        ...
        self.UIElements['define_joints'] = cmds.button(label='Define Joints', width=button_width, height=button_height, bgc=[0.2, 0.2, 0.4], 
            p=self.UIElements['rowColumnLayout1'], command=partial(arm_obj.define_joints, arm_obj.jnt_arm_info, arm_obj.jnt_prefix, arm_obj.jnt_dict))

        self.UIElements['create_arm_joints'] = cmds.button(label='Create Arm Joints', width=button_width, height=button_height, 
        bgc=[0.2, 0.2, 0.4], p=self.UIElements['rowColumnLayout1'], command=partial(arm_obj.create_joints, arm_obj.jnt_dict, arm_obj.jnt_rot))
